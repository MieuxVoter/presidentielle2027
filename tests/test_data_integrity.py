...
def test_hypotheses_csv_well_formed():
    ...
    with hypotheses_csv.open("r", encoding="utf-8") as f:
        ...
        assert required.issubset(set(reader.fieldnames or [])), (
            f"hypotheses.csv missing columns: {required}"
        )
        rows = list(reader)
...
def test_candidats_csv_has_required_columns():
    ...
    with candidats_csv.open("r", encoding="utf-8") as f:
        ...
        assert required.issubset(fieldnames), (
            f"candidats.csv missing columns: {required - fieldnames}"
        )
...
def test_polls_csv_has_required_columns():
    ...
    required = {
        ...
        "debut_enquete", "fin_enquete", "echantillon", "population", "tour"
    }
    ...
    with polls_csv.open("r", encoding="utf-8") as f:
        ...
        fieldnames = set(reader.fieldnames or [])
        assert required.issubset(fieldnames), (
            f"polls.csv missing columns: {required - fieldnames}"
        )
...
def test_all_poll_files_referenced_in_metadata():
    ...
    for poll_file in polls_dir.glob("*.csv"):
        ...
        assert poll_id in poll_ids_in_metadata, (
            f"Poll file {poll_file.name} has no metadata entry in polls.csv"
        )
...
def test_hypothesis_references_in_polls_are_valid():
    ...
    with polls_csv.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if hyp:
                assert hyp in valid_hypotheses, (
                    f"Poll {row.get('poll_id')} references unknown hypothesis: {hyp}"
                )
