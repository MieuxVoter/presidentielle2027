...
def test_le_statut_de_la_pr_reste_une_citation_markdown():
    # Observé sous #194 : le « > » échappé s'affichait « &gt; ⚠️ Aucune PR ».
    body = render.comment(Triage(failed="coupée"), "m", pr="> ⚠️ Aucune PR : tableau <b>écarté</b>")
    assert "\n> ⚠️ Aucune PR" in body
    assert "&gt; ⚠️" not in body and "<b>" not in body
